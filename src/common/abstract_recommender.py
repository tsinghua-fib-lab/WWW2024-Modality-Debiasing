        ...
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            if os.path.isfile(t_feat_file_path):
                self.t_feat = torch.from_numpy(np.load(t_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
            # print(self.v_feat.size()) #4096d
            # print(self.t_feat.size()) #384d

            # self.t_feat = torch.zeros(self.t_feat.size()).cuda()

            # print(self.t_feat.size())
            # nn.init.xavier_normal_(self.v_feat)
            # nn.init.xavier_normal_(self.t_feat)
