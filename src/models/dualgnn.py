...
class DualGNN(GeneralRecommender):
    ...
    def forward(self, protect_type, interaction):
        ...
        ori_v_rep = None
        if self.v_feat is not None:
            ori_v_rep, self.v_preference = self.v_gcn(self.edge_index_dropv, self.edge_index, self.v_feat)
            representation = ori_v_rep
        if self.t_feat is not None:
            ori_t_rep, self.t_preference = self.t_gcn(self.edge_index_dropt, self.edge_index, self.t_feat)
            if representation is None:
                representation = ori_t_rep
            else:
                representation += ori_t_rep
            
        if self.construction == 'weighted_sum':
            if ori_v_rep is not None:
                self.v_rep = torch.unsqueeze(ori_v_rep, 2)
                user_rep = self.v_rep[:self.num_user]
            if ori_t_rep is not None:
                self.t_rep = torch.unsqueeze(ori_t_rep, 2)
                user_rep = self.t_rep[:self.num_user]
            if ori_v_rep is not None and ori_t_rep is not None:
        ...
        if protect_type == 'v':
            weak_pos =  torch.sum(user_tensor * ori_v_rep[pos_item_nodes], dim=1) if ori_v_rep is not None else 0
            weak_neg =  torch.sum(user_tensor * ori_v_rep[neg_item_nodes], dim=1) if ori_v_rep is not None else 0
            strong_pos = torch.sum(user_tensor * ori_t_rep[pos_item_nodes], dim=1) if ori_t_rep is not None else 0
            strong_neg = torch.sum(user_tensor * ori_t_rep[neg_item_nodes], dim=1) if ori_t_rep is not None else 0
        elif protect_type == 't':
            weak_pos =  torch.sum(user_tensor * ori_t_rep[pos_item_nodes], dim=1) if ori_t_rep is not None else 0
            weak_neg =  torch.sum(user_tensor * ori_t_rep[neg_item_nodes], dim=1) if ori_t_rep is not None else 0
            strong_pos = torch.sum(user_tensor * ori_v_rep[pos_item_nodes], dim=1) if ori_v_rep is not None else 0
            strong_neg = torch.sum(user_tensor * ori_v_rep[neg_item_nodes], dim=1) if ori_v_rep is not None else 0
        ...
        return pos_scores, neg_scores, weak_pos, weak_neg, strong_pos, strong_neg

    def calculate_loss(self, protect_type, interaction):
        user = interaction[0]
        pos_scores, neg_scores, weak_pos, weak_neg, strong_pos, strong_neg = self.forward(protect_type, interaction)
        ...
        return loss_value + reg_loss, weak_pos, weak_neg, strong_pos, strong_neg
