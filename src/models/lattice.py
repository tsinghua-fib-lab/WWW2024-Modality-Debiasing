...
class LATTICE(GeneralRecommender):
    ...
    def forward(self, adj, build_item_graph=False):
        if self.v_feat is not None:
            # self.image_embedding = nn.Embedding.from_pretrained(self.v_feat, freeze=False)
            image_adj = build_sim(self.v_feat)
            image_adj = build_knn_neighbourhood(image_adj, topk=self.knn_k)
            image_adj = compute_normalized_laplacian(image_adj)
            self.image_original_adj = image_adj.cuda()
        ...
        if self.t_feat is not None:
            # self.text_embedding = nn.Embedding.from_pretrained(self.t_feat, freeze=False)
            text_adj = build_sim(self.t_feat)
            text_adj = build_knn_neighbourhood(text_adj, topk=self.knn_k)
            text_adj = compute_normalized_laplacian(text_adj)
            self.text_original_adj = text_adj.cuda()
    ...
    def calculate_loss(self, protect_type, interaction):
        ...
        strong_pos = None
        strong_neg = None
        learned_v_adj = compute_normalized_laplacian(self.image_adj)
        item_v_adj = (1 - self.lambda_coeff) * learned_v_adj + self.lambda_coeff * self.image_original_adj
        v_h = self.item_id_embedding.weight
        for i in range(self.n_layers):
            v_h = torch.mm(item_v_adj, v_h)
        v_h = F.normalize(v_h, p=2, dim=1)

        learned_t_adj = compute_normalized_laplacian(self.text_adj)
        item_t_adj = (1 - self.lambda_coeff) * learned_t_adj + self.lambda_coeff * self.text_original_adj
        t_h = self.item_id_embedding.weight
        for i in range(self.n_layers):
            t_h = torch.mm(item_t_adj, t_h)
        t_h = F.normalize(t_h, p=2, dim=1)

        if protect_type == 'v' and self.v_feat is not None and self.t_feat is not None:
            weak_pos =  torch.sum(self.u_emb[users] * v_h[pos_items], dim=1) if self.v_feat is not None else 0
            weak_neg =  torch.sum(self.u_emb[users] * v_h[neg_items], dim=1) if self.v_feat is not None else 0
            strong_pos = torch.sum(self.u_emb[users] * t_h[pos_items], dim=1) if self.t_feat is not None else 0
            strong_neg = torch.sum(self.u_emb[users] * t_h[neg_items], dim=1) if self.t_feat is not None else 0
        elif protect_type == 't' and self.v_feat is not None and self.t_feat is not None:
            # print(out.size())
            # print(ori_t_rep.size())
            weak_pos =  torch.sum(self.u_emb[users] * t_h[pos_items], dim=1) if self.t_feat is not None else 0
            weak_neg =  torch.sum(self.u_emb[users] * t_h[neg_items], dim=1) if self.t_feat is not None else 0
            strong_pos = torch.sum(self.u_emb[users] * v_h[pos_items], dim=1) if self.v_feat is not None else 0
            strong_neg = torch.sum(self.u_emb[users] * v_h[neg_items], dim=1) if self.v_feat is not None else 0
        return batch_mf_loss + batch_emb_loss + batch_reg_loss, weak_pos, weak_neg, strong_pos, strong_neg
