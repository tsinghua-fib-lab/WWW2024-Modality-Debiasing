...
class MVGAE(GeneralRecommender):
    ...
    def forward(self):
        ...
        if 'amazon' in self.dataset:
            ...
            self.result_embed = pd_mu  #v--t--id
    ...
    def calculate_loss(self, protect_type, interaction):
        ...
        if protect_type == 'v' and self.v_feat is not None and self.t_feat is not None:
            weak_pos =  torch.sum(self.result_embed[user] * v_mu[self.n_users+pos_items], dim=1) if self.v_feat is not None else 0
            weak_neg =  torch.sum(self.result_embed[user] * v_mu[self.n_users+neg_items], dim=1) if self.v_feat is not None else 0
            strong_pos = torch.sum(self.result_embed[user] * t_mu[self.n_users+pos_items], dim=1) if self.t_feat is not None else 0
            strong_neg = torch.sum(self.result_embed[user] * t_mu[self.n_users+neg_items], dim=1) if self.t_feat is not None else 0
        elif protect_type == 't' and self.v_feat is not None and self.t_feat is not None:
            weak_pos =  torch.sum(self.result_embed[user] * t_mu[self.n_users+pos_items], dim=1) if self.t_feat is not None else 0
            weak_neg =  torch.sum(self.result_embed[user] * t_mu[self.n_users+neg_items], dim=1) if self.t_feat is not None else 0
            strong_pos = torch.sum(self.result_embed[user] * v_mu[self.n_users+pos_items], dim=1) if self.v_feat is not None else 0
            strong_neg = torch.sum(self.result_embed[user] * v_mu[self.n_users+neg_items], dim=1) if self.v_feat is not None else 0
        return loss_multi + loss_v + loss_t + loss_c, weak_pos, weak_neg, strong_pos, strong_neg
