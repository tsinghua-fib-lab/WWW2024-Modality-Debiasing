...
class SLMRec(GeneralRecommender):
    ...
    def full_sort_predict(self, interaction,item_v_ranking,item_t_ranking,flag,candidate_items=None):
        users = interaction[0]
        ...
        score = torch.matmul(users_emb, items_emb.t())
        ...
        ori_v_feat = deepcopy(self.v_feat)
        self.v_feat = torch.mean(self.v_feat,dim=0,keepdim=True).repeat(self.v_feat.size(0),1)
        ...
        if candidate_items is None:
            items_emb = self.all_items
        score_star_v = torch.matmul(users_emb, items_emb.t())
        self.v_feat = ori_v_feat

        score_star_v = score_star_v * torch.sigmoid(score_v) * torch.sigmoid(score_t)

        #counterfactual-textual
        ori_t_feat = deepcopy(self.t_feat)
        self.t_feat = torch.mean(self.t_feat,dim=0,keepdim=True).repeat(self.t_feat.size(0),1)
        ...
        if candidate_items is None:
            items_emb = self.all_items
        score_star_t = torch.matmul(users_emb, items_emb.t())
        self.t_feat = ori_t_feat
        ...
        if flag >-2:
            ...
            # s_ui_v = torch.sigmoid(-abs(v_fre_ranking_u.cuda())[:,:,0])
            # s_ui_t = torch.sigmoid(-abs(t_fre_ranking_u.cuda())[:,:,0])
            # print(s_ui_v)
            #debias
            # print(s_ui_v)
    ...
    def full_sort_predict_v(self, interaction, candidate_items=None):
        users = interaction[0]
        self.all_users, self.all_items = self.compute()
        users_emb = self.all_users[users]
        scores = torch.matmul(users_emb, self.v_emb_i.t())
        return scores

    def full_sort_predict_t(self, interaction, candidate_items=None):
        users = interaction[0]
        self.all_users, self.all_items = self.compute()
        users_emb = self.all_users[users]
        scores = torch.matmul(users_emb, self.t_emb_i.t())
        return scores
    ...
    def bpr_loss(self, input_users, positive_items, negative_items):
        ...
        users_emb = all_users[input_users]
        pos_emb = all_items[positive_items]
        neg_emb = all_items[negative_items]
    ...
    def calculate_loss(self, protect_type, interaction):
        ...
        if protect_type == 'vt':
            all_users, all_items = self.compute()
            users_emb = self.v_emb_u[users]
            pos_emb = self.v_emb_i[pos]
            neg_emb = self.v_emb_i[neg]
            pos_score = torch.mul(users_emb, pos_emb)
            neg_score = torch.mul(users_emb, neg_emb)
            v_loss = torch.mean(-torch.log(1e-6 + torch.sigmoid(pos_score - neg_score)))

            users_emb = self.t_emb_u[users]
            pos_emb = self.t_emb_i[pos]
            neg_emb = self.t_emb_i[neg]
            pos_score = torch.mul(users_emb, pos_emb)
            neg_score = torch.mul(users_emb, neg_emb)
            t_loss = torch.mean(-torch.log(1e-6 + torch.sigmoid(pos_score - neg_score)))

        return main_loss + self.config['ssl_alpha'] * ssl_loss, v_loss, t_loss
